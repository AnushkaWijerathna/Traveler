"use client"; //used to call login function whenever button is clicked
import { login } from "@/lib/auth-actions";
import Link from "next/link";
import Image from "next/image";
import { Session } from "next-auth";
import { logout } from "@/lib/auth-actions";
//Navbar component...It imports session from layout.tsx as props and its type is Session | null
export default function NavBar({ session }: { session: Session | null }) {
  return (
    <nav className="bg-white shadow-xl py-4 border-b border-gray-200">
      {" "}
        ...
        <div className="flex items-center space-x-4">
          {session ? (
            <>
              ...
              <button
                className="flex items-center justify-center bg-gray-800 hover:bg-gray-900 text-white px-4 p-2 rounded-sm cursor-pointer"
                onClick={logout}
              >
                Sign Out
              </button>
